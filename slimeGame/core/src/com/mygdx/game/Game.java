...
public class Game extends ApplicationAdapter {
	...
	public void create () {
		...
		this.player = new Slime(world, "slime", 250 / ppm, 24 / ppm);
	...
	public void render () {
		...
		batch.begin();
		Texture playerImage;
		if (player.isRight()) {playerImage = playerImageRight;}
		else {playerImage = playerImageLeft;}
		batch.draw(playerImage, player.getBody().getPosition().x*ppm*3.125f - playerImage.getWidth()/2, player.getBody().getPosition().y*ppm*3.125f - playerImage.getHeight()/2 - (camera.position.y - 4.5f)*ppm*3.125f);
		if (player.getBody().getPosition().y > 45) {
			batch.draw(buddy, 13f*ppm*3.125f ,3.925f*ppm*3.125f);
		}
		batch.end();
		if (debug) {
		...
		if (gameOver) {
			...
			font.draw(batch, "Congrats, Bozo", 7*ppm*3.125f, 6f*ppm*3.125f);
			font.draw(batch, "Press r to restart", 4*ppm*3.125f, 4.5f*ppm*3.125f);
			font.draw(batch, "Press q to exit app", 9*ppm*3.125f, 4.5f*ppm*3.125f);
			batch.end();
		}
	}
